...
class Net(nn.Module):
    ...
    def encode_with_intermediate(self, input):
        ...
        return results[1:]
    '''
    def calc_edge_loss(self, input, target, norm = False):
        
        edge_input = cv2.Canny(input,50,220)
        edge_target = cv2.Canny(target,50,220)
        if(norm == False):
          return self.mse_loss(edge_input, edge_target)
        else:
          return self.mse_loss(mean_variance_norm(edge_input), mean_variance_norm(edge_target))
    '''
    ...
    def forward(self, content, style, batch_size):
        ...
        #restoration
        stylized_r = self.transform(g_t_feats[3], content_feats[3], g_t_feats[4], content_feats[4])
        g_t_r = self.decoder(stylized_r)
        g_t_r_feats = self.encode_with_intermediate(g_t_r)
        loss_c_r = self.calc_content_loss(g_t_r_feats[3], content_feats[3], norm = True) + self.calc_content_loss(g_t_r_feats[4], content_feats[4], norm = True)
        ...
        return g_t, loss_c, loss_s, l_identity1, l_identity2, content_contrastive_loss, style_contrastive_loss, loss_c_r